...
1. [リリースページ](https://github.com/6EFB0D/ZipSearch/releases)を開き、ページ下部の **「Assets」** から `ZipSearch-0.5.8-dev-setup.exe` をダウンロード
2. **ZipSearch を終了してから** インストーラを実行する（起動中のままでは上書きできません）
3. デスクトップまたはスタートメニューから ZipSearch を起動

**NTFS**（C:、既定のユーザーフォルダ `LocalAppData` など）: ZipSearch を終了してから setup を実行すれば **上書きインストールできます**。アンインストールは不要です。

**exFAT**（一部の D: や外付け）: 起動した exe が終了後も掴まれたままになり、上書きできないことがあります。次の順で進めてください。

1. ZipSearch をすべて終了する
2. アンインストールだけでは **exe が残ることがある**（これだけでは足りないことがあります）
3. フォルダや exe が消せないときは **再起動**する
4. インストール先フォルダを **手動削除**する
5. setup を実行する

一度も起動していなければ、exFAT でも上書きが通ることがあります。
